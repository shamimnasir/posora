...
const missionCopy = {
  'bangla-starter': ['শব্দ-গোয়েন্দার প্রথম অভিযান', 'ছবির সূত্র ধরে অক্ষর, কার আর শব্দের গোপন পথ খুঁজে বের করো।'],
  'math-play': ['গণিত-শহর বাঁচাও', 'দোকান, মানচিত্র আর সংখ্যার সূত্র মিলিয়ে সঠিক হিসাবের কোড খুঁজে বের করো।'],
  'space-explorer': ['মঙ্গলযানটি নিরাপদে নামাও', 'গ্রহের ক্রম, দূরত্ব আর চাঁদের সূত্র মিলিয়ে মিশন কন্ট্রোলকে সাহায্য করো।'],
  'physics-makers': ['ছোট বিজ্ঞানীর বল-আলো অভিযান', 'ঠেলা, টান, ছায়া আর চুম্বকের প্রমাণ জোগাড় করে যন্ত্রটিকে চালু করো।'],
  'chemistry-lab': ['রান্নাঘরের রহস্য সমাধান', 'নিরাপদে বাছাই, মেশানো আর পর্যবেক্ষণ করে অদৃশ্য পরিবর্তনটি ধরো।'],
  'life-science': ['জীবনের ভিতরের দরজা খোলো', 'বীজ, গাছ, প্রাণী আর শরীরের সূত্র জুড়ে জীবনচক্রের মানচিত্র বানাও।'],
  'nature-guardian': ['বৃষ্টির পানি কোথায় গেল?', 'আকাশ, মাটি, নদী আর পাড়ার প্রাণীদের দেখে প্রকৃতির গোপন পথটি খুঁজে বের করো।'],
  'food-health': ['খাবার গোয়েন্দার প্লেট মিশন', 'রঙ, গন্ধ, লেবেল আর পানির সূত্র মিলিয়ে নিরাপদ সুষম প্লেট সাজাও।'],
  'money-life-skills': ['দোকান-শহরের হিসাব বাঁচাও', 'দাম, খুচরা, প্রয়োজন আর সঞ্চয়ের সিদ্ধান্ত নিয়ে দিনের বাজেট ঠিক করো।'],
  'social-emotional': ['বন্ধুত্বের সেতু মেরামত করো', 'অনুভূতির সংকেত পড়ে, শান্ত হওয়ার পথ বেছে নিয়ে কথার সেতু বানাও।'],
  'discovery-projects': ['আবিষ্কারকের অসম্ভব প্রশ্ন', 'প্রশ্ন থেকে অনুমান, পরীক্ষা আর নতুন নকশায় নিজের সমাধান তৈরি করো.'],
};

const missionPage = (pack, map = false) => {
  const [title, body] = missionCopy[pack.slug] ?? ['আজকের শেখার অভিযান', 'দেখো, ভাবো, বানাও এবং নিজের প্রমাণ খুঁজে বের করো।'];
  const steps = ['দেখো', 'ভাবো', 'করো', 'বলো'];
  return `<section class="page mission-page" style="--accent:${pack.color};--accent2:${palette[2]}">
    <header class="top"><span class="brand">পসরা · ${esc(pack.world)}</span><span class="page-no">${map ? 'মিশন ম্যাপ' : 'মিশন শুরু'}</span></header><div class="orb orb-a"></div><div class="orb orb-b"></div>
    <main><p class="eyebrow">${map ? 'চারটি checkpoint · নিজের গতিতে' : 'একটা গল্প · এগারোটি ছোট কাজ'}</p><h1>${esc(map ? 'আমার মিশন ম্যাপ' : title)}</h1><p class="body">${esc(map ? 'প্রতিটি কাজ শেষ হলে একটি চিহ্ন, রং বা সংখ্যা লিখে রাখো। সব checkpoint জুড়ে শেষে তোমার unlock code তৈরি হবে।' : body)}</p>
    <div class="mission-hero"><div class="mission-orbit">${['★','✦','●','◆'].map((x, i) => `<i style="--n:${i}">${x}</i>`).join('')}<strong>${map ? 'UNLOCK' : 'GO!'}</strong></div><div><b>${map ? 'চার checkpoint, এক বড় আবিষ্কার' : 'তুমি পারবে—এক ধাপ করে এগোও'}</b><p>${map ? 'দেখো → ভাবো → বানাও → ভাগ করো' : 'প্রথমে নিজের অনুমান। শেষে নিজের প্রমাণ।'}</p></div></div>
    <div class="mission-steps">${steps.map((s, i) => `<section><i>${bn(i + 1)}</i><b>${s}</b><span>${map ? 'চিহ্ন: __________' : ['চোখে দেখো', 'একটা প্রশ্ন করো', 'নিজে চেষ্টা করো', 'কাউকে বোঝাও'][i]}</span></section>`).join('')}</div>
    <div class="mission-options"><section><b>পছন্দ করো</b><span>সহজ · সাহসী · বিশেষজ্ঞ</span></section><section><b>বানাও</b><span>এঁকে, কেটে বা জোড়া লাগিয়ে</span></section><section><b>শেষ সূত্র</b><span>৩টি clue = ১টি unlock code</span></section></div>
    <div class="mission-code"><b>${map ? 'শেষ unlock code' : 'আজকের mission token'}</b><span>□</span><span>□</span><span>□</span><span>□</span></div></main><footer><span>পসরা · শেখা হোক খেলার মতো</span><span>নাম: <i></i></span></footer></section>`;
};
...
const bonusWorkspace = (title, j) => {
  if (j === 0) return `<div class="bonus-board poster-board"><div class="poster-hero"><small>কেটে টাঙাও</small><strong>${esc(title)}</strong><span>আজকের ছোট মিশন</span></div><div class="poster-checks">${['দেখেছি', 'করেছি', 'বলেছি', 'আবার করেছি'].map((x) => `<span>□ ${x}</span>`).join('')}</div><div class="toolkit-row"><b>কেটে নাও</b><span>◯ টোকেন</span><span>✦ ব্যাজ</span><span>↻ আবার চেষ্টা</span><span>▣ clue card</span></div><div class="spinner"><b>আজ কী করব?</b><i>দেখো</i><i>মেলাও</i><i>বানাও</i><i>বলো</i></div></div>`;
  if (j === 1) return `<div class="bonus-board routine-board"><div class="visual-title"><b>১৫ মিনিটের পারিবারিক রুটিন</b><span>ঘর · ক্লাস · জোড়ায়</span></div><div class="routine">${[['০–২', 'বেছে নাও'], ['৩–৭', 'নিজে করো'], ['৮–১১', 'খুঁজে/মাপো'], ['১২–১৫', 'বলো ও টোকেন দাও']].map((x) => `<div><i>${x[0]}</i><b>${x[1]}</b><span></span></div>`).join('')}</div><div class="setup-grid"><span>☐ scavenger hunt</span><span>☐ ২ জনের team</span><span>☐ ১টি pencil</span><span>☐ ১টি timer</span></div><div class="adult-note"><b>বড়দের একটি প্রশ্ন</b><p>“তুমি কী দেখে মত বদলালে?”</p></div></div>`;
...
const page = ({ pack, title, eyebrow, body, i, bonus = false, bonusIndex = 0 }) => {
  ...
  return `<section class="page" style="--accent:${accent};--accent2:${accent2}">
    <header class="top"><span class="brand">পসরা · ${esc(pack.world)}</span><span class="page-no">${bn(i + 3)} / ${bn(13)}</span></header>
...
const htmlForPack = (pack) => {
  ...
  </style><style>.mission-page main{padding-top:18mm}.mission-hero{display:grid;grid-template-columns:42mm 1fr;gap:9mm;align-items:center;margin:10mm 0;padding:7mm;border-radius:8mm;background:linear-gradient(135deg,color-mix(in srgb,var(--accent),#fff 84%),#fff);border:1px solid color-mix(in srgb,var(--accent),#9bb0c0 55%)}.mission-hero b{font-size:16pt}.mission-hero p{font-size:11pt;color:#53657f}.mission-orbit{width:38mm;height:38mm;border-radius:50%;display:grid;place-items:center;position:relative;background:radial-gradient(circle at 35% 30%,#fff,var(--accent));box-shadow:0 4mm 0 color-mix(in srgb,var(--accent),#20304a 35%);color:#fff}.mission-orbit strong{font-size:18pt}.mission-orbit i{position:absolute;left:50%;top:50%;transform:rotate(calc(var(--n)*90deg)) translateY(-18mm) rotate(calc(var(--n)*-90deg));font-style:normal;color:var(--accent2);font-size:15pt}.mission-steps{display:grid;grid-template-columns:repeat(4,1fr);gap:3mm}.mission-steps section{min-height:38mm;padding:4mm 3mm;border-radius:5mm;background:#ffffffd9;border:1px solid #d1dce4;display:flex;flex-direction:column;gap:2mm}.mission-steps i{width:9mm;height:9mm;border-radius:50%;display:grid;place-items:center;background:var(--accent);color:#fff;font-style:normal;font-weight:800}.mission-steps b{font-size:12pt}.mission-steps span{font-size:9pt;color:#53657f}.mission-options{display:grid;grid-template-columns:repeat(3,1fr);gap:3mm;margin-top:5mm}.mission-options section{padding:3mm;border-radius:4mm;background:color-mix(in srgb,var(--accent2),#fff 88%);border:1px solid #d1dce4}.mission-options b,.mission-options span{display:block}.mission-options span{font-size:8pt;color:#53657f;margin-top:1mm}.mission-code{display:flex;align-items:center;gap:5mm;margin-top:6mm;padding:4mm 5mm;border-radius:5mm;background:#16233d;color:#fff}.mission-code b{margin-right:auto}.mission-code span{display:grid;place-items:center;width:11mm;height:11mm;border:1px solid #fff;border-radius:3mm;color:var(--accent)}.toolkit-row{display:flex;flex-wrap:wrap;gap:2mm;margin-top:5mm;align-items:center}.toolkit-row b{width:100%;color:var(--accent)}.toolkit-row span{padding:2mm 3mm;border:1px dashed #9bb0c0;border-radius:99px;background:#fff;font-size:9pt}.spinner{width:46mm;height:46mm;border-radius:50%;margin:6mm auto 0;border:5px dotted var(--accent);display:grid;grid-template-columns:1fr 1fr;place-items:center;position:relative;background:conic-gradient(color-mix(in srgb,var(--accent),#fff 70%),#fff,color-mix(in srgb,var(--accent2),#fff 70%),#fff)}.spinner b{position:absolute;top:18mm;font-size:8pt;text-align:center}.spinner i{font-style:normal;font-size:8pt;color:#304561}.setup-grid{display:grid;grid-template-columns:1fr 1fr;gap:2mm;margin-top:5mm}.setup-grid span{padding:2mm 3mm;border-radius:3mm;background:#fff;border:1px solid #d1dce4;font-size:9pt}
  </style></head><body>${missionPage(pack)}${missionPage(pack, true)}${[...activities, ...bonuses].join('')}</body></html>`;
};
