...
const SCENES: Record<string, Builder> = {
  ...
  treeparts({ root, hue }) {
    ...
    return { label: 'বড় হও', anchors, heroScale: 1.05, aim: { y: 1.72, eye: 1.25 }, update(t, _dt, p) {
      const g = 0.3 + p * 0.7;
  ...
  tree({ root, v }) {
    ...
    return { label: v === 'photo' ? 'সূর্যের আলো' : 'বড় হওয়া', groundY: GY, heroScale: 1.05, aim: { y: 1.72, eye: 1.2 }, update(t, dt, p) {
      if (v === 'photo') {
